...
Motivation: 现有情感分析工具主要基于英语或非德语数据集，德语软件工程领域缺乏相关资源。

Method: 从德语开发者论坛提取5,949条语句，由四名德语母语学生基于Shaver情感模型标注六种基本情感。

Result: 标注过程显示高一致性和可靠性，验证了数据集的有效性。现有德语工具在软件工程领域表现不足。

Conclusion: 该数据集为德语软件工程社区提供了有效资源，并讨论了优化标注和扩展应用的潜力。
...
Method: 引入一种工具支持的方法，自动化从用户评论中提取需求和生成解释，并通过工业合作数据集进行评估。

Result: AI生成的需求在相关性和正确性上不如人工，但其生成的解释在清晰度和风格上更受青睐，但仍需人工验证。

Conclusion: 该工作推动了可解释性需求的研究，提供了自动化方法和数据集，同时强调了人工验证的重要性。
...
Result: 提高了工程工作流的效率和可追溯性，支持跨组织协作。

Conclusion: AAS与BPMN的结合为工程工作流提供了高效、安全的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 研究开发者如何将需求文档等信息输入LLMs以生成代码，填补这一未被充分探索的领域。

Method: 访谈了14家公司的18名从业者，分析他们如何利用需求和设计文档为LLMs生成代码提供输入。

Result: 需求文档通常过于抽象，需分解为编程任务并补充设计决策后才能有效用于LLMs。

Conclusion: 即使使用LLMs生成代码，基础的需求工程工作仍不可或缺，研究为自动化需求驱动的SE任务提供了理论支持。
...
<details>
  <summary>Details</summary>
Motivation: 当前LLM在RE任务中存在显著的不确定性和缺乏可控性，缺乏有效的提示方法阻碍了其在RE领域的可信应用。

Method: 采用Kitchenham和Petersen的二次研究协议，筛选并分析了35项主要研究，提出了一种混合分类法，将技术导向的模式与任务导向的RE角色联系起来。

Result: 研究揭示了当前PE4RE的任务、LLM家族和提示类型，并指出了局限性和研究空白。

Conclusion: 提出了一个逐步路线图，将当前的临时PE原型发展为可重复、面向实践者的工作流程。
...
<details>
  <summary>Details</summary>
Motivation: 航天领域需求工程复杂且需高精度，小型机构难以从大量非结构化文档中提取可操作需求，因此探索AI辅助方法以降低参与门槛。

Method: 采用模块化AI驱动方法，预处理航天任务文档，分类语义内容，检索相关标准信息，并利用大语言模型生成需求草案。

Result: 初步应用显示该方法可减少人工工作量，提高需求覆盖范围，并支持轻量级合规性对齐。

Conclusion: 研究为AI在需求工程中的广泛应用提供了路线图，旨在帮助小型机构更易参与大型关键任务。
...
<details>
  <summary>Details</summary>
Motivation: Apple Watch用户对数据处理缺乏选择权，需依赖苹果生态，限制了隐私和数据自主性。

Method: 逆向工程Apple Watch的无线协议，发现安全问题，开发Android版WatchWitch实现互操作性。

Result: 成功实现与Apple Watch的互操作，提供更强的隐私控制和数据自主权。

Conclusion: WatchWitch为智能手表生态系统提供了更多用户选择，增强了对设备的控制。
...
Method: 利用BERT和NLP技术分析安全事件，重构攻击序列并生成测试流程。

Result: 实证验证显示FLOWGUARDIAN在公开威胁报告中表现出高准确性和效率。
...
<details>
  <summary>Details</summary>
Motivation: 反汇编在安全领域中至关重要，但传统方法依赖文件格式假设和启发式规则，导致结果不完整或不准确，尤其是在二进制文件被混淆时。

Method: Disa采用多头自注意力机制学习指令相关性，推断函数入口点和指令边界，并识别与内存块边界相关的指令，以生成更精确的控制流图。

Result: 实验表明，Disa在函数入口点识别上优于现有深度学习方法，F1分数分别提高了9.1%和13.2%，内存块精度提升了18.5%，间接调用目标减少了4.4%。

Conclusion: Disa通过深度学习显著提升了反汇编的准确性和效率，尤其在处理混淆二进制文件时表现出色。
...
Result: 方法能在有损压缩（超过阈值）下保留标记，同时检测篡改块及其位置。
...
<details>
  <summary>Details</summary>
Motivation: IoT生态系统面临复杂恶意软件攻击，传统联邦学习算法在数据异构和类别不平衡时表现不佳。

Method: 使用高斯混合模型生成类原型，添加噪声保护隐私，并通过SMOTE增强少数类表示。

Result: 在N-BaIoT数据集上验证了FedP3E的有效性，减少了统计异构性的负面影响。
...
<details>
  <summary>Details</summary>
Motivation: 研究深度学习在语义通信中的安全性问题，改进传统安全编码方案，以在给定泄漏率约束下最大化传输速率并最小化语义错误概率。

Method: 设计了一种随机打乱特征的语义安全通信系统，将打乱模式作为共享密钥，通过特征序列的排列扭曲语义信息以防止窃听。

Result: 仿真结果表明，该方法在强噪声和不可预测衰落信道中显著提升了安全传输性能。
...
<details>
  <summary>Details</summary>
Motivation: 钓鱼攻击日益复杂，需要高效且准确的检测系统。

Method: 通过实验比较ML、DL和量化LLMs的性能，并研究零样本和少样本提示策略的影响。

Result: LLMs在识别上下文钓鱼线索方面表现潜力，但准确性低于ML和DL。优化后的LLMs（如DeepSeek R1）在17GB VRAM下达到80%以上准确率。

Conclusion: 优化的LLMs有望成为钓鱼防御系统的组成部分，为高效、可解释的AI集成提供路径。
...
<details>
  <summary>Details</summary>
Motivation: 随着网络威胁日益复杂，尤其是在物联网等分布式和资源受限环境中，传统方法难以应对新型攻击模式，需要动态自适应的入侵检测系统。

Method: 提出了一种混合框架，将签名方法的鲁棒性与GPT-2驱动的语义分析相结合。

Result: 实验表明，该模型检测精度提高了6.3%，误报率降低了9.0%，并保持了近实时的响应速度。
...
<details>
  <summary>Details</summary>
Motivation: 关键基础设施系统对社会稳定和经济韧性至关重要，但其日益增长的互联性使其面临多种网络威胁，如勒索软件、DoS攻击和APT。

Method: 研究分析了网络安全漏洞，提出了一种混合AI驱动的框架，结合了实时检测、威胁建模和自动修复功能。

Result: 研究结果为提升关键基础设施的安全性和韧性提供了可行的见解，同时解决了对抗性AI、法规合规性和集成复杂性等问题。

Conclusion: 论文的结论强调了AI在增强关键基础设施网络安全中的重要性，并提出了未来研究和实践的方向。
...
<details>
  <summary>Details</summary>
Motivation: 研究现有提示注入防御方法的实际安全性，揭示其潜在漏洞。

Method: 构建优化攻击算法，针对SecAlign和StruQ两种防御进行白盒攻击测试。

Result: 攻击成功率高达70%，且攻击成本仅需适度增加令牌数。

Conclusion: 现有防御方法在对抗优化攻击时表现不足，需进一步改进安全性。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是打破VANETs中的隐私保护假名方案，提升车辆追踪能力，尤其是在攻击者无法完全覆盖车辆路径的现实场景中。

Method: 方法是通过结合DSRC和Wi-Fi探针请求信号，使用三种指标（Count、Statistical RSSI和Pearson RSSI）关联假名和Wi-Fi标识符。

Result: 实验结果表明，Pearson RSSI指标在所有场景下对假名更换方案的追踪效果最佳。

Conclusion: 结论是RADAR算法显著提升了追踪能力，并公开了所有实现和仿真场景。
...
<details>
  <summary>Details</summary>
Motivation: 图像传感器在安全和安防关键系统中至关重要，但其视觉数据的完整性可能受到电磁攻击的威胁，目前缺乏针对此类物理层攻击的防御措施。

Method: 通过精心调制的电磁干扰，攻击者可以在图像传感器中诱导彩虹色伪影，并分析这些伪影对目标检测模型的影响。

Result: 攻击导致目标检测模型出现显著误判，揭示了视觉感知系统中未被充分探索的漏洞。
...
<details>
  <summary>Details</summary>
Motivation: 生成AI提供者需要确保其生成内容的水印安全，防止用户伪造水印以虚假指控提供者。

Method: 提出了一种多密钥扩展方法，适用于任何水印技术和模态，并提供了理论保证。

Result: 实验证明该方法显著降低了伪造水印的成功率。

Conclusion: 多密钥扩展方法有效减轻了水印窃取攻击，为生成AI内容的安全性提供了保障。
...
Result: 实际部署显示99.9%的合规性，交易量大且隐私保障强。

Conclusion: 该框架为去中心化经济中的代理协作提供了信任基础，实现了全球互操作的代理互联网。
...
<details>
  <summary>Details</summary>
Motivation: 钓鱼攻击利用人类行为绕过技术防御，现有警告对话框因解释不清和静态内容效果有限。

Method: 通过大规模用户研究（N=750）比较人工和两种LLM（Claude 3.5 Sonnet和Llama 3.3 70B）生成的解释效果，分析两种解释风格（基于特征和反事实）对行为指标和感知结果的影响。

Result: LLM生成的解释在降低钓鱼易感性上表现优异，Claude效果尤为突出；基于特征的解释对真实钓鱼更有效，反事实解释减少误报率。
...
<details>
  <summary>Details</summary>
Motivation: 研究Android应用中硬件安全存储的使用情况，了解开发者对敏感数据的保护措施及其性能影响。

Method: 分析了490,119个Android应用，结合Play Store的数据安全标签，评估硬件安全存储的使用情况，并测试了不同硬件支持的加密操作性能。

Result: 多数应用未充分利用硬件安全存储，高级安全元件在加密操作中性能较差。

Conclusion: 硬件安全存储的普及和性能仍需改进，以更好地保护敏感数据。
...
<details>
  <summary>Details</summary>
Motivation: FHE仅支持一维向量操作，多维张量操作需要复杂的打包和映射，现有系统抽象层次过多，难以调试和优化。

Method: 利用einsum符号明确编码张量结构和操作，将其分解为FHE友好的操作序列，并实现EinHops系统。

Result: EinHops能够简单、通用且可解释地执行加密张量操作，从简单转置到复杂多维收缩均可实现。

Conclusion: einsum符号的显式特性使得FHE张量系统更透明和易于理解，EinHops为开发者提供了完整的底层打包策略可见性。
...
<details>
  <summary>Details</summary>
Motivation: 针对大型语言模型（LLM）系统在外部数据交互时面临的提示注入攻击，现有测试时防御方法效果不佳，作者提出一种更有效的解决方案。

Method: 提出DefensiveToken，通过插入并优化特殊令牌的嵌入，在需要时增强安全性，同时保持实用性。

Result: DefensiveToken在安全性和实用性之间实现灵活切换，接近训练时防御的效果。
...
Result: 在案例研究1中，GPT-4o和GPT-4o-mini在180个随机生成的FSMs中实现100%有效路径成功率；案例研究2中，LLM控制器在非线性动态处理中表现与经典PID控制相当。

Conclusion: 通过结构化反馈和模块化代理，LLMs能够统一高级符号规划和低级连续控制，为化学工程中的弹性语言驱动自动化铺平道路。
...
<details>
  <summary>Details</summary>
Motivation: 当前大型语言模型在游戏交易系统中存在规则违反问题（如物品幻觉和计算错误），导致玩家信任度下降。
...
Method: 采用神经符号方法，结合问题树查询大型语言模型（LLM），比较人工与自动特征提取的效果。

Result: 提出了一种系统评估方法，用于量化新闻文章的相关性，并比较人工与机器分类的差异。

Conclusion: 神经符号方法在稀疏数据条件下有效，为供应链非法活动检测提供了新思路。
...
Method: 系统由多个专门化智能体组成，分别执行检索、编码、结果解释和输出评审等任务，并能本地执行代码。采用规划与控制策略协调工作流。

Result: 成功应用于博士级别的宇宙学任务（利用超新星数据测量宇宙学参数），在基准测试中表现优于现有最先进的LLM。
...
<details>
  <summary>Details</summary>
Motivation: 多智能体强化学习中的高效探索问题复杂且具有挑战性，本研究旨在通过专家探索方法解决这一问题。
...
Method: ViDove模仿人类翻译流程，整合多模态记忆系统和长短时记忆模块，结合领域知识提升翻译性能。

Result: ViDove在字幕生成和通用翻译任务中表现优异，BLEU分数提升28%，SubER提升15%。

Conclusion: ViDove通过多模态输入显著提升了翻译质量，并推出了新的评测基准DoveBench。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLMs的广泛应用，防止其生成有害内容成为重要问题，研究旨在探讨过滤方法的有效性及其计算限制。

Method: 通过理论分析，研究输入提示和输出过滤的计算可行性，并基于密码学假设证明其局限性。
...
<details>
  <summary>Details</summary>
Motivation: 解决大语言模型在科学领域（如药物发现）中推理能力受限的问题，克服传统方法的高计算成本或数据利用不足的缺点。
...
Method: StarDojo包含1,000个任务，分为五大领域，并提供100个代表性任务子集。它提供统一、用户友好的界面，支持多操作系统和并行环境实例。

Result: 现有最先进的MLLM代理表现有限，最佳模型GPT-4.1的成功率仅为12.7%，主要因视觉理解、多模态推理和低级操作挑战。
...
Method: 提出AlgEval框架，结合案例研究分析注意力模式和隐藏状态。

Result: 揭示了LLM中涌现的搜索算法，为算法解释性提供基础。

Conclusion: 系统评估LLM算法有助于高效训练和新型架构设计，推动可解释性研究。
...
Result: 研究发现常用工具生成的规则集普遍存在负面特征。

Conclusion: 规则模型的解释需要更严格的分析以避免误导。
...
<details>
  <summary>Details</summary>
Motivation: 现有GNN模型在知识图谱链接预测中表现有限，尤其是对未见实体的归纳设置。

Method: 提出Context Pooling方法，首次将图池化应用于知识图谱，并设计邻居精度和召回率指标筛选逻辑相关邻居。

Result: 在三个公开数据集上应用于两个SOTA模型，48个设置中42个达到SOTA性能。
...
<details>
  <summary>Details</summary>
Motivation: 通过自动化数据提取提高疫苗安全监测效率，及早发现疫苗接种后的不良事件。

Method: 使用提示工程创建标注数据集，并由人工确认。比较提示工程模型、微调模型和基于规则的方法。

Result: 微调的Llama 3模型在提取疫苗名称的准确性上优于其他模型，量化技术使其在资源有限环境中高效部署。

Conclusion: 大型语言模型在自动化急诊笔记数据提取方面具有潜力，可支持高效的疫苗安全监测。
...
Result: 数值结果展示了该框架在链式结构中的优势和局限性，为其在信用网络中的实际应用提供了见解。

Conclusion: 该框架为链式信用网络中的信念推理提供了高效且鲁棒的方法，同时揭示了其适用范围和潜在改进方向。
...
<details>
  <summary>Details</summary>
Motivation: 评估LLMs在几何和空间推理方面的能力，特别是在处理室内设计约束和布局扰动时的表现。

Method: 使用符号化格式（如JSON、XML）编码室内场景，设计多样化问题类型测试度量、拓扑推理及设计约束。

Result: LLMs在浅层查询中表现良好，但在模拟物理约束、保持空间一致性及布局扰动下的泛化能力上表现不佳。

Conclusion: PlanQA揭示了当前LLMs在真实世界布局推理中的不足，为未来语言模型在空间和几何推理方面的改进提供了方向。
...
<details>
  <summary>Details</summary>
Motivation: DPO虽然在实际应用中表现良好，但其理论性质和内在局限性尚未充分研究，尤其是其对初始化的敏感性和概率质量分配问题。

Method: 通过概率演化的视角分析DPO的动态特性，并提出一种结合监督微调和改进DPO目标的双层优化框架，引入正则化以稳定优化过程。

Result: 实验表明，该方法在推理和摘要任务中显著提高了准确性，并更好地对齐输出分布与预期偏好，优于标准DPO。
...
<details>
  <summary>Details</summary>
Motivation: 研究小提琴制作中的尺寸标准化对乐器几何特征的影响，尤其是轮廓线的变化。

Method: 使用25把小提琴的3D几何网格数据，提取轮廓线并拟合抛物线曲线，计算参数特征，应用分类方法。

Result: 发现几何特征可以一定程度上预测小提琴是否被缩小，其中开口参数beta最具预测性。

Conclusion: 几何分析可用于区分缩小与非缩小的小提琴，但存在一定难度，尤其是对部分改造的乐器。
...
<details>
  <summary>Details</summary>
Motivation: 传统AI评估聚焦技术能力或避免危害，而FAI Benchmark旨在衡量AI如何促进人类全面繁荣。
...
<details>
  <summary>Details</summary>
Motivation: 现有混合专家模型（MoE）需要大量训练数据和复杂优化，而人类驾驶员的学习过程更高效。受此启发，作者提出技能导向的MoSE模型，以更高效地实现自动驾驶任务。

Method: 提出技能导向路由机制，定义和标注特定技能，构建层次化技能数据集，并通过单次前向过程整合辅助任务（如描述、推理、规划）。

Result: MoSE模型在CODA AD任务中表现优于多个8B+参数模型，激活参数量减少至少62.5%，且性能达到SOTA。

Conclusion: MoSE通过模仿人类驾驶员的学习和推理过程，显著提升了自动驾驶系统的效率和性能，同时降低了计算成本。
...
<details>
  <summary>Details</summary>
Motivation: 当前AI依赖大规模模型和数据集，导致环境、经济和伦理成本高，难以持续和普及。生物感官系统的动态适应性启发了这一研究。

Method: 提出自适应感知，动态调整传感器参数（如曝光、灵敏度、多模态配置），以减少协变量偏移并提升效率。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型在法律纠纷分析中存在法律知识表示不足、概念理解有限和推理缺陷等问题，需要一种更有效的解决方案。

Method: 提出三阶段分层提示结构和三层知识图谱架构，结合四种互补的法律概念检索方法，并与网络搜索技术集成。

Result: 实验结果显示，该框架在法律纠纷分析中性能显著提升，能够准确分析复杂案例并理解司法决策逻辑。
...
<details>
  <summary>Details</summary>
Motivation: 探讨AI模型性能不平等问题，并提出计算规模扩大带来的边际收益递减将导致性能收敛。

Method: 开发模型分析计算规模对性能的影响，结合基准数据和理论模型验证。

Result: 计算规模扩大带来的边际收益递减，使得预算有限的模型性能接近最佳模型。
...
Conclusion: AI在信息处理和沟通密集型职业中表现最佳，实际使用与预测相符。
