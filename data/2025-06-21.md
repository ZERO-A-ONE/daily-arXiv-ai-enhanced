...
Method: 构建OS-Harm基准，包含150个任务测试三类危害，并提出自动化评估方法。

Result: 测试显示前沿模型易受故意滥用和提示注入攻击，且偶尔执行不安全行为。

Conclusion: OS-Harm为计算机使用代理的安全性提供了标准化评估工具，揭示其潜在风险。
...
Result: 研究发现不准确绘图普遍存在，图形计算错误是主要原因，视觉语言模型在检测错误中的效果因提示而异（29%-57%）。

Conclusion: 研究为数据可视化库的自动化测试提供了基础，并指出了视觉语言模型在错误检测中的潜力与局限性。
...
Method: 通过分析25项灰盒模糊测试研究，提取了7个与控制和数据流相关的程序特征，并生成了一个包含153个程序的基准测试集。

Result: 评估了11种流行的模糊测试工具，结果显示其性能受程序特征及其强度显著影响。
...
TL;DR: 该论文研究了利用大语言模型（LLM）自动预测增强报告（ER）的批准情况，发现LLM在准确性和召回率上优于传统方法，并提出了改进方向。


<details>
  <summary>Details</summary>
Motivation: 手动处理增强报告耗时且易遗漏有价值建议，因此需要自动化工具提升效率。

Method: 系统评估了18种LLM变体（包括编码器和解码器模型），并与传统方法（如CNN/LSTM-BERT/GloVe）对比。

Result: LLM在准确性和召回率上显著优于传统方法，尤其是LoRA微调的Llama 3.1 8B Instruct模型达到79%准确率。
...
Conclusion: 该验证框架为分布式程序提供了一种有效的静态分析方法，确保通信竞争的不存在。
...
<details>
  <summary>Details</summary>
Motivation: 支持软件开发者在敏捷/Scrum开发中的冲刺计划和回顾活动，提升项目管理的效率和效果。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLMs的发展，其在单元测试自动化中的应用日益增多，但缺乏系统性总结，本文旨在填补这一空白。

Method: 通过分析相关论文，从单元测试和LLMs的角度分类任务，讨论模型使用、适应策略和混合方法。

Result: 总结了LLMs在单元测试中的关键挑战和未来研究方向。

Conclusion: 本文为单元测试社区提供了系统性概述，促进未来研究。
...
<details>
  <summary>Details</summary>
Motivation: 代码片段文档化对开发者和用户至关重要，尤其是第三方库。研究旨在了解开发者常用的描述类型，并评估LLM（如Llama）在生成描述中的能力。

Method: 使用NPM代码片段数据集（185,412个包，1,024,579个片段），选取400个样本进行手动分类和LLM评估。

Result: 55.5%的原始描述基于示例，LLM准确识别79.75%的示例描述。生成描述的平均相似性得分为0.7173，显示相关性但需改进。
...
TL;DR: 本文提出了一种基于抽象语法树（AST）的代码分块方法（\ourwork），以解决现有基于行的分块方法破坏语义结构的问题，显著提升了代码生成任务的表现。


<details>
  <summary>Details</summary>
Motivation: 现有的基于行的分块方法在代码检索增强生成（RAG）中常破坏语义结构，影响生成质量，因此需要一种结构感知的分块方法。

Method: 提出了一种基于AST的分块方法，递归地将大AST节点分解为小分块，并在大小限制内合并兄弟节点，生成语义连贯的单元。
...
<details>
  <summary>Details</summary>
Motivation: 大型语言模型（LLMs）的预训练依赖于难以管理的大规模文本数据，现有方法依赖训练数据的记忆，而模型提供者试图限制这种记忆。本文旨在探索一种不依赖记忆的间接数据投毒方法。
...
<details>
  <summary>Details</summary>
Motivation: 现有FDE协议的证明者和验证者运行时间与文件长度线性相关，效率较低。本文旨在通过RS编码和加密技术，将运行时间降至接近常数。

Method: 将文件视为速率1的RS码字，扩展为低速率RS码，加密后仅对少量随机密文子集进行正确性证明，利用RS解码修复损坏符号。

Result: 协议显著降低了运行时间，同时保持了客户端和服务器的公平性，仅增加了可调的通信冗余开销。

Conclusion: 改进后的FDE协议在效率和公平性上均有显著提升，并通过zk-SNARK技术解决了比特币实现中的椭圆曲线不匹配问题。
...
<details>
  <summary>Details</summary>
Motivation: 当前CVMs的威胁模型未涵盖物理层保护和侧信道攻击，用户无法准确评估物理攻击风险，需改进认证机制以提供端到端安全保障。

Method: 提出利用受保护平台标识符（PPID）绑定CVM与提供商，并讨论TEE认证的扩展与强化。

Result: 现有TEE认证实现差异大，验证和迁移困难，需解决这些挑战以推动CVMs的采用。

Conclusion: 需扩展TEE认证机制以绑定CVM与提供商，解决实现差异带来的挑战，提升CVMs的安全性。
...
<details>
  <summary>Details</summary>
Motivation: 现有基于矩阵机制的最优算法在无界设置下无法直接应用，因为其隐私保证依赖于预先知道输入规模。

Method: 引入基于对数扰动的新型矩阵分解方法，优化了误差和计算效率。

Result: 算法在无界设置下实现了平滑误差，方差为O(log^{2+2α}(t))，空间和时间复杂度分别为O(t)和O(log t)。

Conclusion: 新算法在误差和效率上优于现有方法，且在实际应用中表现接近最优。
...
<details>
  <summary>Details</summary>
Motivation: AI/ML医疗设备的快速发展带来了显著的网络安全风险，可能危及患者安全，因此需要在设计阶段就解决这些问题。

Method: 通过分析设备召回、不良事件和已知漏洞数据，设计了一套工具和技术，用于上市前风险评估。

Result: 提出了一套帮助制造商将网络安全作为核心设计原则的工具，以提升设备安全性。

Conclusion: 强调在AI/ML医疗设备设计阶段解决网络安全问题的紧迫性，以确保患者安全。
...
<details>
  <summary>Details</summary>
Motivation: 传统加密接口设计需要上下文切换，效率低下，MECHA旨在解决这一问题。

Method: 采用多线程架构（Server、Client、Transceiver线程及Sender/Receiver队列），通过UNIX域套接字实现多请求并发处理。

Result: 实验结果显示，MECHA比传统接口设计快83%，适用于多种通信协议。

Conclusion: MECHA在安全通信领域（如云计算和物联网）具有广泛应用潜力，提供更高效的并发加密操作解决方案。
...
Method: 采用CNN、LSTM和GRU相结合的混合深度学习模型，充分利用各架构的优势。
...
<details>
  <summary>Details</summary>
Motivation: 物联网应用的快速增长对边缘计算的高效、高吞吐和节能数据处理提出了更高要求，传统CPU加密方法在延迟敏感和资源受限环境中存在性能瓶颈。
...
Method: 利用卷积自编码器（CAE）检测干扰，并通过CWGAN-GP生成平衡数据集以解决类别不平衡问题。

Result: CAE在干扰检测中表现优异，平均精确度为97.33%，召回率为91.33%，F1分数为94.08%，准确率为94.35%。

Conclusion: CAE在复杂数据环境下表现出鲁棒性，优于其他基准模型（CDAE和CSAE）。
...
Method: 提出flexHEGs系统，包括可审计的保证处理器和安全外壳，支持开源、灵活且可更新的验证功能。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI能力快速发展，国际安全面临新挑战，flexHEGs通过硬件保障提供可信的AI治理方案。

Method: 研究flexHEGs在限制扩散、安全规范、军事AI风险管理及战略稳定中的应用，分析验证型和规则型两种治理模型。

Result: 博弈论分析表明，全面的flexHEG协议在合理假设下具有稳定性，能有效管理AI风险。

Conclusion: flexHEGs为国际AI治理提供了可行的技术基础，但需克服协调和技术门槛等挑战。
...
Method: 提出安全的转换管道，将标量输入映射到向量和矩阵空间，并提供原子协议支持线性与非线性安全计算。

Result: 实验表明，EVA-S2PMLP在保持高推理精度的同时，显著降低通信开销，性能提升达12.3倍。

Conclusion: 该框架在金融、医疗等领域具有实用性，能同时保障数据隐私和模型效用。
...
<details>
  <summary>Details</summary>
Motivation: 现有防御方法难以恢复已中毒的全局模型，而重新训练成本高且可能影响隐私和一致性。

Method: 通过近似Hessian矩阵的线性计算和秘密共享技术保护历史更新，结合客户端准备、周期性恢复更新和最终精确更新。

Result: 实验表明，恢复的全局模型性能接近完全重新训练的模型，但计算和时间成本显著降低，同时保护了本地模型隐私。
...
Result: 在27个真实Rust库中，deepSURF重新发现了20个已知漏洞和6个新漏洞。

Conclusion: deepSURF在检测能力上优于现有工具，显著提升了漏洞发现的效率。
...
<details>
  <summary>Details</summary>
Motivation: 大语言模型的语义参数具有不透明性和弱可解释性，适合用于数据加密传输。

Method: 通过大模型的确定性微调中间结果作为传输内容，适用于如网关内传输等场景。

Result: 提出了一种基于大模型的通信加密方法，理论上可适用于任何大模型。

Conclusion: CipherMind为通信加密提供了一种新范式，具有广泛的应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM能力的扩展，其安全风险日益严重，需要系统梳理越狱攻击和防御机制的研究现状。

Method: 通过分类主流越狱技术、分析攻击方法和防御策略，并结合数据集和评估指标，提供结构化理解。

Result: 总结了现有研究的局限性，如对代理特定安全问题的关注不足、混合越狱方法分类不清晰等，并提出了未来研究方向。

Conclusion: 研究旨在增强对越狱机制的理解，推动更强大和适应性强的防御策略的发展。
...
<details>
  <summary>Details</summary>
Motivation: 研究GPT-4在识别软件漏洞方面的效果，并与传统SAST工具对比。

Method: 通过分析多种安全错误，评估GPT-4在漏洞扫描中的表现。

Result: GPT-4（高级数据分析）在检测32种可利用漏洞时准确率达94%，优于SAST工具。

Conclusion: GPT-4在漏洞扫描中具有潜力，但需结合安全设计和其他AI安全最佳实践。
...
<details>
  <summary>Details</summary>
Motivation: 在原始设施位置问题中，差分隐私算法的近似比下界为Ω(√n)，而现有方法采用超集假设可能损害用户隐私。本文旨在证明这一下界不适用于改进版本。

Method: 提出了一种基于LDP的算法，确保用户在特定位置的存在隐私，并实现了常数近似比和小附加因子。

Result: 实验结果表明，该算法在合成和真实数据集上均优于直接方法。

Conclusion: 改进的设施位置问题在LDP框架下可以实现高效且隐私保护的解决方案，突破了原始问题的限制。
...
Result: 攻击显著降低任务完成率，学术环境中攻击成功率极高，且模型规模与安全能力正相关。

Conclusion: RAS-Eval揭示了LLM代理部署中的关键风险，为未来安全研究提供了基础框架。
...
<details>
  <summary>Details</summary>
Motivation: 医疗数字化带来了巨大好处，但也成为攻击目标，难以保障安全。

Method: 使用FPGA满足实时性和功耗限制，通过整体系统评估实现性能。

Result: 系统能够实时检测恶意客户端。

Conclusion: 提出的方法在医疗数字化环境中有效提升了安全性。
...
<details>
  <summary>Details</summary>
Motivation: 硬件木马（HT）可能导致未经授权的软件执行或特权操作访问，因此需要一种高效的检测方法。

Method: 采用基于汉明单纠错（HSEC）架构的硬件安全检查器（HSC）来检测HT激活。

Result: 实验结果显示，该方法对潜在HT激活的检测率达到100%，无假阳性或漏检，且硬件开销极低。

Conclusion: 该方法高效且低开销，适用于实时检测硬件木马，保障微处理器安全。
...
<details>
  <summary>Details</summary>
Motivation: 现有数据流神经网络加速器易受恶意攻击者通过侧信道攻击逆向工程，威胁知识产权安全。

Method: 采用无监督降维技术减少计算开销，结合轻量级分类器恢复折叠和量化参数。

Result: 攻击阶段仅需337毫秒恢复硬件参数（精度>95%），421毫秒完全恢复参数（平均4条轨迹），计算效率显著优于现有方法。

Conclusion: 该方法在更现实的攻击场景中表现优异，计算效率提升显著，且无需轨迹平均即可获得更好结果。
...
<details>
  <summary>Details</summary>
Motivation: 解决Raz提取器因计算时间过长而不实用的问题。

Method: 提出改进版本的Raz提取器，降低熵需求并优化计算时间。

Result: 实现了准线性计算时间，提供强和量子证明版本，并开源代码实现。
...
<details>
  <summary>Details</summary>
Motivation: 钓鱼网站通过欺骗性结构和社交工程逃避检测，现有单代理分类方法存在幻觉和缺乏可解释性问题。

Method: PhishDebate采用四个专门代理分析网页的不同文本方面（URL结构、HTML组成、语义内容和品牌冒充），由协调员和法官进行结构化辩论。

Result: 在真实钓鱼数据集上，PhishDebate召回率和真阳性率均达98.2%，优于单代理和CoT基线。

Conclusion: PhishDebate通过模块化设计和多代理辩论显著提升了钓鱼检测的准确性和适应性。
...
<details>
  <summary>Details</summary>
Motivation: 低地球轨道卫星的激增推动了地球观测任务的发展，但多卫星系统的自主协调仍是一个关键挑战，传统优化方法难以满足实时决策需求。

Method: 采用强化学习和多智能体强化学习框架，模拟单卫星和多卫星操作，解决能源、数据存储和部分可观测性问题，并评估了多种MARL算法。
...
<details>
  <summary>Details</summary>
Motivation: 经典二值逻辑系统无法捕捉人类决策的细微差别，且在多模态环境中需要人工标注，而神经网络虽能提取多模态数据的丰富信息，但缺乏可解释的推理结构。

Method: CALM通过领域树表示谓词，利用神经网络迭代优化其模拟真值，并通过符号推理模块确保约束满足。

Result: 在填空物体放置任务中，CALM准确率达92.2%，优于经典逻辑（86.3%）和LLM基线（59.4%），并生成符合逻辑约束和人类偏好的空间热图。
...
Method: 采用语言驱动探索和基于粒子群优化的反馈引导更新，从零构建代理系统并联合优化功能与协作。

Result: 在六个真实任务中表现优异，尤其在TravelPlanner基准上相对ADAS提升了261.8%。

Conclusion: SwarmAgentic为可扩展和自主的代理系统设计迈出了重要一步，结合了群体智能与全自动多代理生成。
...
<details>
  <summary>Details</summary>
Motivation: 现有持续学习基准测试主要在CPU上运行，导致计算瓶颈和任务序列长度受限，且缺乏针对多智能体协作环境的持续学习基准。

Method: 提出MEAL基准，利用JAX实现GPU加速，支持在普通PC上快速运行100个任务的序列，并通过实验分析现有方法的局限性。

Result: 实验显示，简单结合现有CL和MARL方法在简单环境中表现良好，但在复杂环境中无法持续协调和适应。

Conclusion: MEAL为CMARL研究提供了高效基准，并揭示了改进架构和算法的重要性。
...
<details>
  <summary>Details</summary>
Motivation: PPO在长序列生成任务中效率低下，硬件利用率不足，影响了LLMs的训练效率。

Method: 提出T-PPO，包括EGAE优势估计和独立优化策略与价值模型的机制，减少冗余计算。

Result: 在AIME 2024上验证，T-PPO训练效率提升2.5倍，性能优于现有方法。
...
<details>
  <summary>Details</summary>
Motivation: 传统启发式算法依赖人工设计且难以泛化，HeurAgenix旨在通过LLM自动化和优化启发式策略。

Method: HeurAgenix分为启发式进化和动态选择两阶段，利用LLM提取策略并选择最优解，支持轻量级模型以降低成本。
...
<details>
  <summary>Details</summary>
Motivation: 海上物联网（MIoT）的计算需求快速增长，但任务不确定性导致计算卸载和资源分配效率低下，需要无人机和船舶协作解决。

Method: 提出协作MEC框架，利用Lyapunov优化处理任务不确定性，将问题转化为马尔可夫博弈，并设计异构智能体软演员-评论家算法求解。

Result: 仿真结果表明，所提方法能有效解决计算卸载和资源分配问题。

Conclusion: 通过无人机和船舶协作及优化算法，成功解决了海上物联网任务不确定性问题，提升了计算效率。
...
<details>
  <summary>Details</summary>
Motivation: 现有导航方法通常同时处理所有历史观测数据，忽略了数据内部关联结构，限制了任务性能的进一步提升。
...
<details>
  <summary>Details</summary>
Motivation: 故障分析（FA）是一个复杂且知识密集型的过程，随着AI模型的增多，如何协调这些组件以高效集成到FA流程中成为挑战。
...
<details>
  <summary>Details</summary>
Motivation: LLM在动态环境中作为决策者表现良好，但其无状态特性需要自然语言历史表示。以往研究对游戏历史的编码方式缺乏系统性，影响了行为分析和研究间的可比性。

Method: 提出了一个框架，从三个维度（动作信息性、奖励信息性和提示风格）系统化构建状态表示，并应用于动态自私路由游戏中。

Result: 研究发现，提供总结性历史表示、后悔信息而非原始收益、以及有限他人动作信息的表示，能更接近博弈论均衡预测，行为更稳定。

Conclusion: 自然语言状态表示对LLM代理行为有显著影响，特定表示方式能优化其决策表现，接近理论均衡。
...
Conclusion: AI政策模块是有效的教学工具，能提升学生对AI伦理和政策的理解与实践能力。
...
Method: 提出了两种方法：1）Efficiency Steering（无需训练的激活导向技术）；2）Self-Rewarded Efficiency RL（动态平衡任务准确性和简洁性的强化学习框架）。
...
Method: 开发了统一的仿真平台（Embodied Web Agents task environments）和基准测试（Embodied Web Agents Benchmark），涵盖多种跨领域任务。

Result: 实验显示现有AI系统与人类能力存在显著差距，揭示了结合物理认知与网络知识访问的挑战与机遇。

Conclusion: 论文为AI代理的跨领域智能研究提供了新方向，并公开了数据集、代码和平台。
