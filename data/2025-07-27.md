...
Motivation: 研究AI工具（如GitHub Copilot和ChatGPT）在日常软件工程任务中的应用，尤其是开发者如何信任、改进或拒绝AI生成的内容。

Method: 基于实践者报告和土耳其、阿塞拜疆三个行业环境的直接观察，提出了一个过程模型和2D决策框架。
...
<details>
  <summary>Details</summary>
Motivation: 解决科学报告中标准化和透明度不足的问题，评估自动化工具在提升严谨性和透明度方面的效果。

Method: 对11种自动化工具在9种严谨性标准上进行广泛比较。

Result: 某些工具在特定标准上表现优异，工具组合在某些标准上表现更佳，并提出了工具开发的改进方向。

Conclusion: 提出了对严谨性和透明度检测工具开发的见解和建议，相关代码和数据已公开。
...
<details>
  <summary>Details</summary>
Motivation: 生成式AI在游戏设计中的应用日益广泛，但缺乏对其在开源社区中实际应用的实证研究。

Method: 构建开源游戏仓库数据集，对GitHub问题进行开放式卡片分类和主题分析，比较GenAI、TradAI和非AI话题。

Result: 揭示了GenAI在开源游戏开发中的使用模式、开发者关注点及集成实践。
...
<details>
  <summary>Details</summary>
Motivation: 通过将MITRE ATT&CK攻击技术与P-SSCRM任务映射，帮助软件组织更有效地管理供应链风险。

Method: 采用四种独立策略确定映射关系，并将P-SSCRM任务与10个其他框架任务关联。

Result: 提供了MITRE ATT&CK与其他政府及行业框架之间的映射关系。

Conclusion: 该映射为软件供应链攻击的风险管理提供了实用工具。
...
<details>
  <summary>Details</summary>
Motivation: 项目式学习（PjBL）能提升学生动机、参与度及多项技能，但教师采用率不一致，需探究障碍及促进策略。

Method: 采用混合方法，通过在线调查收集80名计算教师的定量和定性数据，进行统计和主题分析。

Result: 结果显示PjBL虽受重视，但采用受限于规划、项目设计及机构支持不足；同行协作、专业发展和机构激励是促进因素。
...
<details>
  <summary>Details</summary>
Motivation: 尽管GHA提供了官方文档和社区最佳实践，但缺乏对开源项目中实际CI工作流是否符合这些实践的实证研究。

Method: 通过分析Java、Python和C++仓库中的大量GHA工作流数据，研究其结构、复杂性、异质性和合规性。

Result: 预计发现最佳实践的强遵循区域和需要改进的领域，同时揭示不同编程语言在CI设计上的差异。

Conclusion: 研究结果将为CI服务提供改进建议，强调更清晰的指南和全面的文档示例的重要性。
...
<details>
  <summary>Details</summary>
Motivation: 标识符名称对程序理解至关重要，但相似名称可能导致误解和认知负担。

Method: 开发了一个分类法，用于分类标识符名称相似性的不同形式。

Result: 初步发现标识符名称相似性在软件项目中的存在，并提出了分类法。

Conclusion: 分类法为研究标识符名称相似性对代码理解和协作的影响提供了基础，未来可进一步扩展。
...
Method: 收集3,859个真实LLM应用，分析依赖关系，提取1,555个风险问题，进行实证分析。

Result: 发现LLM应用存在深度依赖和供应链漏洞，强调全面安全分析的必要性。

Conclusion: 建议研究人员和开发者采取综合安全措施，构建更可信的LLM系统。
...
Method: 引入NoCode-bench基准，包含634个任务和114k代码变更，验证LLMs在文档更新与代码实现配对任务中的表现。
...
<details>
  <summary>Details</summary>
Motivation: 高质量的元数据对实现FAIR原则至关重要，但手动创建耗时耗力。

Method: SMECS从GitHub等在线仓库提取元数据，并提供交互界面供用户整理和导出为CodeMeta文件。

Result: 可用性实验证实SMECS提供良好的用户体验。
...
<details>
  <summary>Details</summary>
Motivation: 旨在缩短合规性和重新设计周期，减少ADAS相关功能的开发和测试时间。

Method: 利用大型语言模型（LLMs）进行需求模型化总结、测试场景生成、仿真代码和目标平台代码生成，并采用检索增强生成（RAG）优化测试场景生成。

Result: 生成测试场景代码、仿真环境和目标平台实现，同时通过模型驱动工程（MDE）检查需求一致性。
...
<details>
  <summary>Details</summary>
Motivation: 传统模糊测试技术难以生成合理的模糊驱动程序，且无条件执行浪费资源。

Method: 利用LLM理解库使用方式，提取API组合约束，并采用双调度框架管理API组合和模糊驱动程序。

Result: 在33个真实库中测试，Scheduzz显著减少计算开销，覆盖率和漏洞发现优于现有技术。

Conclusion: Scheduzz在库模糊测试中表现出色，提高了覆盖率和漏洞发现效率。
...
Result: YATE在6个开源项目上平均覆盖32.06%更多代码行，杀死21.77%更多突变体，优于其他LLM方法。
...
<details>
  <summary>Details</summary>
Motivation: 研究目标是开发一种适用于所有类型问题的文件定位方法，而非仅限于特定问题（如缺陷）。

Method: 提供了一种数据管道，用于创建问题文件定位数据集，并评估了传统信息检索方法的性能。

Result: 结果显示，针对特定问题的启发式方法在通用问题上表现不佳，且不同问题类型之间存在显著性能差异。

Conclusion: 研究鼓励开发可适应项目特定特征的通用模型。
...
Method: 开发了一种新框架，通过FMI连接SystemC虚拟平台与外部工具，并以温度传感器为例展示了数据交互。

Result: 该框架允许未修改的目标软件在虚拟平台上运行，并接收来自外部工具的实时环境数据，从而支持更广泛的软件测试。
...
<details>
  <summary>Details</summary>
Motivation: 手动代码审查主观且耗时，自动化需求迫切，但现有大型语言模型缺乏逻辑推理能力。

Method: 结合符号推理技术与大型语言模型，测试了多种模型（如CodeT5、CodeBERT等）在CodexGlue数据集上的表现。

Result: 混合方法显著提升了自动化代码审查的准确性和效率。

Conclusion: 符号推理与大型语言模型的结合是自动化代码审查的有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 解决开源与闭源代码库之间的分布偏移问题，验证RAG在工业级代码库中的实用性。

Method: 在微信代码库中测试26个开源LLM，比较标识符RAG和相似性RAG，并分析词法和语义检索技术的影响。
...
Method: 通过分析混沌工作负载（如DDoS攻击）对网络功能的影响，结合内核监控和实证评估方法。

Result: 发现多样化资源配置对保障SLA合规性至关重要，内核监控在安全防御中具有可扩展性。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（如SliceLocator）依赖规则识别漏洞路径，泛化能力有限，需改进以更准确地解释漏洞根因。

Method: 提出基于GNN的模型，捕捉语义和句法依赖以识别潜在漏洞终点（PSPs），并通过程序切片提取和排序漏洞路径。
...
Method: 使用Coq证明助手和Verifiable C的分离逻辑实现，对libsecp256k1的模逆实现进行验证。

Result: 完成了对libsecp256k1模逆实现的计算机验证证明。

Conclusion: 通过形式化验证确保了新算法的正确性，降低了潜在错误风险。
...
<details>
  <summary>Details</summary>
Motivation: FPGA的高层次综合（HLS）在实现时序收敛和设计优化时面临挑战，缺乏对平台特定pragma的自动化支持。

Method: TimelyHLS通过LLM和RAG生成并迭代优化HLS代码，结合FPGA特定知识库和合成日志进行改进。

Result: 实验显示，TimelyHLS减少手动调整70%，提升性能达4倍，资源节省超50%。
...
<details>
  <summary>Details</summary>
Motivation: 钓鱼邮件通过欺骗性内容和恶意负载对网络安全构成威胁，现有机器学习模型的性能依赖于训练数据的质量和多样性。

Method: 整合了135,894个样本，涵盖多种钓鱼策略和合法邮件，并设计了多种特征。使用四种分类模型（RF、XGB、MLP和CNN）进行实验评估。
...
<details>
  <summary>Details</summary>
Motivation: 研究旨在暴露黑盒模型水印系统中被忽视的漏洞，以提醒防御策略的改进。

Method: 提出两种逆向工程方法：1) 通过特殊输入图像逆向HNet的逆模型；2) 利用等效加性特性逆向HNet的前向代理模型。

Result: 两种攻击方法均实现100%水印去除成功率，图像质量优异（最高PSNR达34.69 dB）。

Conclusion: 研究强调了黑盒模型水印系统的脆弱性，亟需开发更鲁棒的防御策略。
...
<details>
  <summary>Details</summary>
Motivation: 现有DNN水印方法具有侵入性，可能导致模型行为变化和额外调优成本，限制了实际应用。
...
TL;DR: 论文提出了一种基于量子随机数的改进版ChaCha密码（QRE-ChaCha），通过引入量子随机数增强扩散性，提升了抗差分攻击能力，同时保持了原版的高效性。


<details>
  <summary>Details</summary>
Motivation: 随着AI辅助密码分析和量子计算技术的发展，传统ChaCha密码的安全实现面临挑战，因此需要增强其安全性。

Method: 设计通过将初始常数与量子随机数异或，并在奇数轮中定期注入量子随机数到选定状态字中，以增强扩散性。

Result: 改进后的QRE-ChaCha表现出更强的抗差分攻击能力，生成的密钥流具有统计随机性，并通过了NIST和GM/T 0005-2021的随机性测试。

Conclusion: QRE-ChaCha在保持高效的同时显著提升了安全性，适用于密码学应用。
...
<details>
  <summary>Details</summary>
Motivation: 随着电网数字化，网络安全威胁增加，现有智能电网网络靶场多为一次性专有系统，缺乏灵活性且成本高昂。

Method: 定义基于XML的SG-ML建模语言，结合IEC 61850配置语言，开发工具链自动生成功能化智能电网网络靶场。

Result: 通过案例研究验证了Auto-SGCR的有效性，工具链和示例模型已开源。

Conclusion: Auto-SGCR框架提升了智能电网网络靶场的可配置性、可访问性和可重复性，为网络安全测试和专家培训提供了高效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有研究误以为LoRA微调数据对成员推理攻击（MIA）免疫，但预训练模型可能导致更多信息泄露。

Method: 引入LoRA-Leak框架，整合15种MIA方法（10种现有和5种改进），评估LoRA微调模型的隐私风险。

Result: 实验表明LoRA微调模型仍易受MIA攻击（如AUC达0.775），并提出有效防御措施（如dropout）。

Conclusion: 预训练模型加剧LoRA微调模型的MIA风险，需关注数据隐私保护。
...
<details>
  <summary>Details</summary>
Motivation: 随着巴西LGPD和欧盟GDPR的实施，数据匿名化成为软件开发中的关键问题，需要评估其合规性和实用性。

Method: 研究了聚合、泛化、扰动和k-匿名等技术，并将其应用于包含个人和敏感数据的数据集。

Result: 分析显示不同方法的有效性存在显著差异，需平衡隐私与数据实用性。

Conclusion: 数据匿名化技术需根据具体需求选择，以实现合规性和实用性的最佳平衡。
...
<details>
  <summary>Details</summary>
Motivation: 随着技术的普及和硬件能力的提升，数字证据在调查中的比重增加，但处理大量数据耗时且繁琐，需要减少误报。

Method: Scout采用大语言模型处理文本信息，多模态模型分析多媒体文件，快速识别潜在证据。

Result: Scout能高效识别对调查有价值的证据文件，减少人工筛选时间。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLMs的快速发展和普及，微调对齐模型的安全性变得至关重要，但现有研究表明，即使是看似无害的数据也可能导致安全对齐退化。

Method: 提出LARF方法，通过分析LLM中安全敏感的层，检测微调数据中的安全隐患。

Result: 实验证明LARF能有效识别含安全隐患的数据，移除后缓解了安全对齐退化。

Conclusion: LARF为微调LLMs时的安全性提供了一种有效解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 在流体力学中，理解流动物理机制与准确预测同样重要。传统机器学习方法缺乏可解释性，而SR能揭示复杂系统中的数学关系，无需先验模型假设。
...
<details>
  <summary>Details</summary>
Motivation: 现有系统在数据分析中忽视了结构化推理过程，导致推理步骤缺乏一致性。
...
<details>
  <summary>Details</summary>
Motivation: 传统高级规划系统（APS）因定制和维护成本高昂而难以普及，SmartAPS旨在提供更易访问的解决方案。

Method: 利用工具增强的LLM构建对话系统，支持自然语言查询、反事实推理、推荐和场景分析。

Result: SmartAPS为操作规划师提供了直观的交互方式，提升了操作管理的灵活性和效率。
...
<details>
  <summary>Details</summary>
Motivation: 定性时间线规划的规划存在问题是PSPACE完全的，但非确定性自动机无法直接用于合成策略，需确定性化步骤。本文旨在找到可直接映射到确定性自动机的片段。

Method: 识别定性时间线规划的一个片段，其规划存在问题可直接映射到确定性有限自动机的非空问题，并确定适合该片段的Allen关系的最大子集。

Result: 提出了一个可直接映射到确定性有限自动机的规划片段，并确定了适合该片段的Allen关系的最大子集。
...
<details>
  <summary>Details</summary>
Motivation: 探讨AI如何超越模仿实现真正创造力，提出“创意潜力隐藏在失败中”的观点。
...
<details>
  <summary>Details</summary>
Motivation: 探讨图表是否对AI系统分析数据有帮助，填补AI与可视化结合的研究空白。

Method: 使用GPT 4.1和Claude 3.5两种商业视觉语言模型，在三种代表性分析任务中测试图表对性能的影响。

Result: 图表显著提升了模型对合成数据描述的精确性和准确性，尤其在复杂数据集中。
...
<details>
  <summary>Details</summary>
Motivation: 解决AI研究受限于人类认知能力的瓶颈，推动AI自主创新。

Method: 提出ASI-Arch系统，实现端到端的自主研究，包括假设生成、代码实现、训练与验证。

Result: 在20,000 GPU小时内完成1,773次实验，发现106种创新架构，性能超越人类设计。

Conclusion: ASI-Arch为AI自主研究提供了蓝图，证明科学发现可通过计算扩展。
...
Method: 采用模块化任务代理系统，处理结构化和非结构化数据，自动完成特征选择、模型选择和预处理推荐。

Result: 在老年医学、姑息治疗和结肠镜成像的公开数据集上验证了框架的有效性，实现了无需人工干预的自动化流程。

Conclusion: 该框架减少了机器学习生命周期中的专家干预需求，为临床环境中的AI应用提供了可扩展且经济高效的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 由于临床试验中安全数据收集时间有限，且疫苗广泛接种后需要快速监测潜在安全问题，研究旨在通过NLP和主动学习技术提升疫苗安全信号的监测效率。

Method: 结合NLP、主动学习和数据增强技术，开发分类器以分析急诊科分诊记录，优化标注过程并提高模型性能。

Result: 研究提出了一种更准确、高效的分类器，能够减少误报并快速识别疫苗安全问题。

Conclusion: 该分类器为疫苗安全监测提供了及时、有效的工具，尤其在紧急情况下具有重要应用价值。
...
Conclusion: 均值GNNs的表达能力受设置和假设影响，灵活性较高但均匀设置下较弱。
...
<details>
  <summary>Details</summary>
Motivation: 受双系统认知理论启发，区分知识推理对模型分析、可解释性和发展至关重要。

Method: 通过快速思维和慢速思维两种认知模式，分解知识检索（Phase 1）和推理调整（Phase 2），量化贡献。

Result: 结果显示推理调整具领域特异性，参数扩展提升知识和推理能力，知识主要分布在低层网络，推理在高层。

Conclusion: 框架为理解LLMs提供了新视角，并对扩展规律、知识编辑和小模型推理限制提供了新见解。
...
<details>
  <summary>Details</summary>
Motivation: 研究析取在Answer Set Programming中的非最小化语义，探索不同方法的等价性和强度。
...
Result: 提供了一个逻辑化的AI风险分析基础，支持对高风险AI系统和通用AI系统的操作性评估。
...
Conclusion: 该方法有效，但项重写系统的开发具有挑战性。
...
<details>
  <summary>Details</summary>
Motivation: 金融领域需要LLMs不仅能做出最优决策，还需在不确定性下生成创造性、合理的未来情景，现有基准主要关注事实准确性或逐步逻辑，无法满足需求。

Method: 开发ConDiFi基准，包含发散思维（宏观金融提示）和收敛思维（多跳对抗性选择题）两部分，评估14个领先模型。

Result: GPT-4o流畅性高但在新颖性和可操作性上表现不佳；DeepSeek-R1和Cohere Command R+在生成可操作见解方面表现优异。
...
Method: 提出IB-aware推理优化（IBRO）框架，推导出令牌级代理目标，并设计轻量级IB正则化方法。

Result: 在多个数学推理基准测试和RL算法中验证了IB正则化的有效性，推理性能持续提升。

Conclusion: IBRO框架为LLM推理提供了理论基础，轻量级IB正则化方法简单高效，无需额外计算开销。
...
Method: 比较了基于模型（VI）和无模型（PPO）的方法，使用MDP框架和SBR模型。
...
TL;DR: 本文探讨了如何利用现代GPU的计算能力增强Compact-Table（CT）算法，以处理大规模表约束问题，并展示了其在实际应用中的有效性。
...
Result: 实验结果表明，GPU加速的CT算法在处理大规模表约束问题时表现显著优于传统CPU方法。

Conclusion: 通过GPU加速，CT算法能够有效处理大规模表约束问题，为实际应用提供了可行的解决方案。
...
<details>
  <summary>Details</summary>
Motivation: 概念探测用于解释神经网络，但现有研究忽视了训练数据的影响。
...
Result: 在安全性基准测试中平均提升46.54%，优于GPT-4.1和Claude Opus 4，同时保持通用能力。

Conclusion: SafeLadder框架能构建稳健、可靠且可信的通用AI，证明安全与能力可协同进化。
