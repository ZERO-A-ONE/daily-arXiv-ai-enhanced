...
Motivation: 技术面试对软件工程师求职至关重要，但复杂且难以准备，现有教育体系支持不足。

Method: 通过调查131名正在准备技术面试的候选人，分析其准备方法和教育的作用。

Result: 候选人很少在真实场景中训练，课程未能有效支持准备，导致压力与准备不足。
...
<details>
  <summary>Details</summary>
Motivation: 开发者常用关键词和正则表达式搜索代码，但结构化搜索工具使用DSL语言，学习成本高。自然语言查询可以降低门槛。

Method: 结合LLM的自然语言理解能力和结构化搜索工具，将自然语言查询转换为DSL查询，并在Semgrep和GQL上实现。

Result: 在400个Java项目查询的基准测试中，方法表现优异，精确率和召回率达55%-70%，显著优于基线方法。

Conclusion: 自然语言查询结合LLM和结构化搜索工具是一种高效、易用的代码搜索方法。
...
Method: 使用446个开源Android应用的数据集，提取代码指标和元数据，评估回归和分类模型。

Result: 回归模型效果差，但分类模型表现显著提升，最佳F1分数为0.72。
...
<details>
  <summary>Details</summary>
Motivation: 传统上依赖自我报告工具评估人类因素（如幸福感和压力）可能存在偏差，因此研究探索结合更客观的生理测量方法。

Method: 实验包括预调查、编程任务（佩戴生物传感器）、简短后调查和退出访谈。

Result: 心理测量工具未检测到压力，访谈中参与者报告了无压力或时间压力的混合感受，生物特征数据仅在EDA相位峰值上显示显著差异。

Conclusion: 通过更严格时间限制诱导压力的方法不足，为未来研究提供了方法学启示。
...
<details>
  <summary>Details</summary>
Motivation: 现有情感分析工具在不同平台数据集上表现不一致，影响了团队动态分析和需求工程中的AI驱动分析。

Method: 分析5个平台的10个数据集的统计和语言特征，评估14种工具，提出基于特征的映射方法和问卷。

Result: 数据集特征可改进工具选择，Transformer模型表现稳定但效果仍依赖上下文。

Conclusion: 提出的方法支持选择可信工具，但需持续评估以适应通信环境变化。
...
<details>
  <summary>Details</summary>
Motivation: COBOL代码维护困难，缺乏文档，现有LLM方法因COBOL的独特挑战（如超出LLM窗口限制）效果不佳。

Method: 采用多智能体方法，两个LLM智能体协作，结合代码库上下文信息生成代码解释。

Result: 在14个开源COBOL项目上验证，方法在函数、文件和项目级别的解释效果显著优于基线。

Conclusion: 多智能体方法有效解决了COBOL代码解释的挑战，提升了生成解释的质量和覆盖范围。
...
<details>
  <summary>Details</summary>
Motivation: Python中的类型错误常导致运行时故障，现有静态分析工具误报率高，而单元测试生成技术缺乏针对性指导。

Method: RTED结合逐步类型约束分析和反射验证，指导测试生成过程并抑制误报。

Result: 在BugsInPy和TypeBugs基准测试中，RTED比现有技术多检测22-29个类型错误，误报率降低173.9%-245.9%，并在6个开源项目中发现12个未知错误。

Conclusion: RTED在检测Python类型错误方面表现出色，显著优于现有技术，具有实际应用潜力。
...
<details>
  <summary>Details</summary>
Motivation: 当前公平性测试方法依赖人工和固定模板，资源密集且难以扩展，亟需自动化解决方案。

Method: 采用变形测试（metamorphic testing）和LLM生成测试用例及评估输出，设计了14种变形关系和开源工具支持。

Result: Meta-Fair在12个预训练LLM上测试，平均精度92%，发现29%的执行存在偏见，LLM评估F1-score最高0.79。

Conclusion: Meta-Fair为LLM公平性测试提供了高效自动化路径，尽管非确定性影响一致性，但通过设计可缓解，未来潜力巨大。
...
Method: 研究采用零样本提示和最少到最多提示两种方法优化LLMREI，并在33次模拟访谈中评估其性能。微调方法因初步表现不佳被放弃。

Result: LLMREI在减少访谈错误、提取需求和生成上下文相关问题上表现与人类相当，尤其适合大规模利益相关者访谈。

Conclusion: LLMREI在自动化需求获取方面具有潜力，尤其适用于大规模访谈场景，但需进一步优化。
...
<details>
  <summary>Details</summary>
Motivation: 自适应CPS需要更有效的人机协作，但现有反馈循环（如MAPE-K）未能充分整合人类操作者，且HMT涉及隐私和伦理问题。

Method: 开发新方法将HMT融入CPS反馈循环，并构建框架以在系统生命周期中验证伦理和人类价值。

Result: 提出了整合HMT的方法和伦理验证框架。

Conclusion: 研究为自适应CPS中实现无缝HMT提供了理论和实践基础，强调了伦理和隐私的重要性。
...
Result: 发现合作机会，并提出术语映射方法。

Conclusion: 系统化方法为未来众包扩展和验证奠定了基础。
...
Method: RLHGNN将事件日志转化为异构图，包含三种边类型，并通过强化学习动态选择最优图结构，使用异构图卷积进行预测。

Result: 在六个真实数据集上，RLHGNN表现优于现有方法，且推理延迟约为1毫秒，适用于实时监控。
...
Result: 标志减少了误分类，提高了识别准确性和性能，且比定义更易理解。
...
<details>
  <summary>Details</summary>
Motivation: 小型组织和初创公司缺乏法律专业知识，确保软件合规性是一项资源密集型任务。现有方法未能充分考虑法律元数据间的关联性，且依赖手动标注或启发式机器学习，难以推广到新文档。

Method: 采用文本蕴含和上下文学习技术，设计了一个领域特定的Python类结构作为元模型，自动生成法律文本的规范表示，减少对大规模标注数据的依赖。

Result: 在13个美国州的数据泄露通知法律上测试，生成的表示通过89.4%的测试用例，精确率和召回率分别为82.2和88.7。

Conclusion: 该方法有效减少了手动标注需求，提高了对新法规的适用性，为法律合规性分析提供了高效工具。
...
<details>
  <summary>Details</summary>
Motivation: 需求访谈中，访谈者面临认知负荷、信息过载和领域知识不足等挑战，需要实时生成高质量问题。LLM（如GPT-4o）在自然语言处理任务中的优异表现为其应用提供了潜力。

Method: 基于常见访谈错误类型框架，利用GPT-4o生成后续问题，并通过两个对照实验评估LLM生成问题与人工问题的表现。

Result: LLM生成的问题在清晰度、相关性和信息量上不劣于人工问题，且在错误类型指导下表现更优。

Conclusion: LLM可实时提升需求访谈质量，减轻访谈者负担，具有实际应用潜力。
...
Method: MGC框架通过Malware Description Intermediate Representation（MDIR）将高级恶意意图与看似无害的代码片段连接起来，实现模块化分解和规避对齐生成。

Result: 实验表明，MGC在三个基准数据集上生成的恶意软件正确性显著优于越狱方法和地下服务（分别提升365.79%和78.07%），并能复现并增强16个真实恶意软件样本。
...
<details>
  <summary>Details</summary>
Motivation: 零权限传感器（如加速度计）的广泛使用可能导致隐私泄露，但现有方法（如静态分析和动态分析）存在开发复杂、需要root权限等问题，用户难以监控传感器使用情况。

Method: 开发了ARMOUR，一种用户空间运行时监控工具，利用Android的采样率变化和收敛行为，无需复杂开发链或root权限。

Result: 在1,448个商业应用中的评估显示，ARMOUR能有效检测混淆代码中的传感器使用，并发现50%看似无关的应用访问多个零权限传感器。

Conclusion: ARMOUR为用户提供了简便的传感器监控方式，同时支持安全研究，但Android政策变化和技术问题仍需进一步解决。
...
<details>
  <summary>Details</summary>
Motivation: 传统$c$-差分均匀性方法中，乘法操作破坏密码结构（如密钥加法），限制了其实际应用。本文旨在解决这一问题，提出一种新的内部$c$-差分方法。

Method: 开发内部$c$-差分方法，将乘法作用于输入而非输出，并证明其与外部$c$-差分均匀性的对偶关系。构建统计-计算框架，对9轮Kuznyechik密码进行截断$c$-差分分析。

Result: 在9轮Kuznyechik密码中，通过数百万差分对的统计分析，发现显著非随机性，偏差比达1.7倍，校正p值低至1.85×10^-3，表明其安全性不足。

Conclusion: 本文提出的内部$c$-差分方法解决了传统技术的局限性，并首次实现了对9轮Kuznyechik密码的实际区分攻击，揭示了其潜在安全风险。
...
<details>
  <summary>Details</summary>
Motivation: 硬件TRNG在硬件安全和密码系统中的重要性，以及保护DNN模型参数的隐私和完整性的需求。

Method: 利用DRAM单元在RowHammer干扰下的随机比特翻转作为熵源，设计EIM-TRNG框架，并应用于DNN权重数据的加密。

Result: 验证了DRAM熵提取的有效性，提供了一种低成本、高安全性的硬件保护方案。

Conclusion: EIM-TRNG为机器学习模型的硬件级安全保护提供了新方向。
...
<details>
  <summary>Details</summary>
Motivation: 结合环签名的强匿名性和同态签名的可计算性，适用于需要匿名数据来源和可验证同态计算的场景。

Method: 基于格的线性同态环签名方案，在标准模型下基于小整数解（SIS）假设证明安全性。

Result: 方案在完全密钥暴露下实现强匿名性，并能抵抗内部腐败攻击。

Conclusion: 该方案为后量子安全提供了一个统一框架，推动了隐私增强的同态计算发展。
...
Conclusion: BOLAZ是首个基于系统最佳实践授权逻辑的防御方法，具有实际部署的实用性。
...
<details>
  <summary>Details</summary>
Motivation: 探究是否可以通过操纵LLM内部激活来绕过对齐机制，获取隐私相关查询的敏感信息。

Method: 使用轻量级线性探针识别与隐私拒绝行为相关的注意力头，并通过操纵这些头的激活诱导模型生成非拒绝响应。

Result: 实验显示95%的越狱成功率，50%的响应泄露真实个人信息，包括敏感属性和其他隐私数据。

Conclusion: LLM中记忆的隐私信息可通过针对性操纵内部激活提取，揭示了潜在的安全风险。
...
<details>
  <summary>Details</summary>
Motivation: 日志分析在网络安全中至关重要，能够为网络和系统威胁检测提供信息源。

Method: 比较了三种开源LLM在零样本、少样本提示和微调策略下的表现，并映射检测到的威胁到MITRE CAPEC，定义物联网特定缓解措施。

Result: LLM在多类攻击分类中优于基线模型，并能提供检测与缓解建议的联合指导。

Conclusion: 微调LLM在物联网安全日志分析中具有潜力，可提升异常检测和缓解建议的效果。
...
<details>
  <summary>Details</summary>
Motivation: 传统IDS/IPS系统生成的警报数量庞大且误报率高，现有机器学习方法和单通道RAG管道无法有效解决问题。

Method: 采用模块化代理框架，结合细粒度分类器、工具适配器和迭代检索-推理循环，动态优化威胁标签和解释。

Result: CyberRAG在每类攻击中达到94%以上的准确率，总分类准确率为94.92%，生成的解释在BERTScore和专家评估中表现优异。

Conclusion: CyberRAG通过代理设计和语义协调，实现了高检测准确性和可信的解释，为半自主网络安全工作流提供了可行方案。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法主要关注探测请求，忽视了管理帧的动态特性，导致在高密度移动环境中性能受限。

Method: 将设备行为建模为有限状态机，利用矩阵编码状态转移频率和时延特征，生成紧凑特征向量进行相似性比较。

Result: 在多样化真实环境中，非随机化设备的识别准确率超过86%，通过时间聚合进一步提升性能。

Conclusion: 该方法能够在大规模环境下唯一且一致地表征设备，优于现有技术。
...
<details>
  <summary>Details</summary>
Motivation: 为替代ASCON等密码中的5位替换，设计更安全的10位S盒。

Method: 采用Feistel网络和5细胞自动机生成子置换，通过经验选择规则和布局。

Result: 生成的10位S盒安全性优于或媲美AES S盒。

Conclusion: 该S盒可替代现有密码中的5位替换，提升安全性。
...
<details>
  <summary>Details</summary>
Motivation: 解决披萨生产中数据安全和实时监控的需求，确保食品安全和合规性。

Method: 使用物联网设备实时监测温湿度，区块链确保数据安全，树莓派处理数据并触发警报。

Result: 实验显示改善了原料管理，减少浪费，提高厨房效率。

Conclusion: 系统有效提升了披萨生产的透明度和响应速度，支持合规和审计。
...
<details>
  <summary>Details</summary>
Motivation: 随着联网车辆的数字化发展，安全风险增加，但受限于安全、成本和伦理问题，攻击场景数据稀缺，亟需高效生成高质量攻击数据的方法。

Method: 开发了一种上下文感知的攻击数据生成器，利用参数化攻击模型和CAN消息解码技术，生成多种攻击类型的数据，并通过深度学习模型进行验证。

Result: 生成的攻击数据在入侵检测系统（IDS）案例中表现出高效性和可扩展性，深度学习模型展现出高检测和分类能力。

Conclusion: 该方法能有效生成高质量攻击数据，支持车辆网络安全研究，同时提供了数据真实性和应用方面的见解。
...
Method: 手动构建验证规则，但缺乏系统性方法确保其鲁棒性。

Result: 验证规则易受漏洞利用，需系统性方法识别缺陷。

Conclusion: 需开发系统性方法以提升验证规则的鲁棒性和安全性。
...
<details>
  <summary>Details</summary>
Motivation: 随着LLM代理应用的普及，其安全问题尚未得到充分研究，尤其是邮件代理的安全机制。

Method: 提出EAH攻击方法，通过外部邮件资源覆盖代理的原始提示，并开发EAHawk工具进行大规模评估。

Result: 实验评估了1,404个邮件代理实例，全部被成功劫持，平均仅需2.03次攻击尝试即可控制一个实例。
...
<details>
  <summary>Details</summary>
Motivation: 量化LDP扰动对复杂或黑盒分类器效用的影响是一个理论挑战，需要一种通用框架来解决。

Method: 将LDP扰动视为集中在原始数据周围的分布函数，并将分类器视为黑盒函数，通过鲁棒性分析量化效用。

Result: 框架在典型机制-分类器组合的案例研究中表现良好，理论量化与实证观察一致，尤其在低维输入空间中。

Conclusion: 该框架为LDP机制和隐私参数的选择提供了理论支持，分段机制在常见场景中表现更优。
...
<details>
  <summary>Details</summary>
Motivation: 开源模型对AI安全社区至关重要，通过开放研究共同开发攻击与防御方法，推动对抗提示注入攻击的科学进展。

Method: 采用改进版SOTA SecAlign防御方法，提供完整的训练细节，并在通用指令调优数据集上进行训练。

Result: 在9个实用基准和7个安全基准上表现优异，Meta-SecAlign-70B在对抗提示注入攻击上达到SOTA鲁棒性，且通用性能与闭源商业LLM相当。

Conclusion: Meta SecAlign为开源社区提供了高性能且安全的LLM选择，推动了对抗提示注入攻击的研究与实践。
...
Result: 当前模型在标准条件下难以隐藏短消息，但在额外条件下可能成功；初步显示隐写推理能力。
...
Method: 通过分析公开信息、实验GPU内核模块，并提出合理推测，揭示系统组件细节。
...
Method: STELLA采用多代理架构，包括动态工具库和模板库，能够自主发现和整合新工具。
...
Method: HCVR采用贪心算法，通过反向消除和多数投票规则，利用相关性阈值决定特征的保留或剔除。
...
Result: 研究揭示了推理性能与计算资源之间的权衡，并强调了TTC方法的实用性、适应性和可扩展性。
...
Method: 使用系统生物学标记语言（SBML）编码的生物系统模型进行干实验室模拟，生成模拟数据，评估六种前沿LLM在137个小系统和总共350个系统上的表现。

Result: 性能更强的模型表现更好，但随着系统复杂性增加，所有模型的性能显著下降，表明LLM在科学能力方面仍有很大改进空间。

Conclusion: SciGym为评估LLM的科学能力提供了可行方法，但当前模型在复杂系统上的表现仍需提升。
...
<details>
  <summary>Details</summary>
Motivation: 现代AI模型训练成本高且固定，而动物能快速适应环境变化，尤其是社交物种。研究旨在探索如何从神经科学中获取灵感，改进AI的持续学习和适应能力。

Method: 整合AI中的持续学习和上下文学习文献，以及神经科学中关于行为任务规则、奖励概率或结果变化的研究。

Result: 提出了一种议程，说明神经科学如何指导AI发展，以及AI如何反过来促进神经科学研究。

Conclusion: 通过双向学习，NeuroAI领域有望推动AI和神经科学的共同进步。
...
<details>
  <summary>Details</summary>
Motivation: 研究动机是解决AI系统（如招聘算法）中的偏见问题，传统方法（如重采样）因数据选择偏差和标签偏差而效果有限。
...
Result: DTS方法在GSM8K和MATH上分别提升7.1%和4.2%，计算成本仅增加1.03倍，优于MCTS。

Conclusion: 结构化探索多样化问题解决方法能更有效地生成偏好数据，优于传统方法。
...
Conclusion: 该算法展示了在建模不同博弈场景中的多样性和基准潜力，为博弈理论研究提供了新视角。
...
Method: 引入NL2FLOW系统，生成自然语言、结构化中间表示和PDDL的规划问题，并评估LLM生成的计划。

Result: 最佳模型在生成有效计划和最优计划上分别达到86%和69%的成功率，直接推理表现更优。
...
<details>
  <summary>Details</summary>
Motivation: 现有信念修正研究多依赖假设，这些假设仅描述修正必须做什么，而忽略了修正能够做什么。论文旨在探讨修正机制的能力，如灵活性、可塑性等，以满足不同应用场景的需求。

Method: 通过分析多种修正机制（如词典序、自然修正、激进修正等），论文探讨了它们是否具备特定能力（如可塑性、平等化、教条化等）。

Result: 研究表明，不同修正机制具备不同的能力，例如某些机制可以实现教条化信念状态，而另一些则不能。

Conclusion: 信念修正机制的能力分析比单纯依赖假设更为重要，不同应用场景需要不同能力的修正机制。
...
<details>
  <summary>Details</summary>
Motivation: 现有LLM方法在关键词生成中依赖大规模数据，缺乏在线多目标优化和质量控制，限制了其自动化能力。
...
Method: 采用AI原生设计，结合模型、实验和仪器的协同进化，管理仪器、制定实验流程和优化策略。
...
TL;DR: 研究通过提升任务清晰度增强大语言模型的推理能力，在Coq定理证明中验证。
...
<details>
  <summary>Details</summary>
Motivation: 加速科学进步，通过自动化机器学习模型的设计、实现和训练，提升AI研究代理在真实世界问题中的表现。

Method: 将AI研究代理形式化为搜索策略，设计并测试不同的操作符集和搜索策略（贪婪、MCTS、进化算法），分析其相互作用。
...
Conclusion: 责任属性在集体决策中的计算复杂性具有明确的层级结构，为相关研究提供了理论基础。
...
Method: 基于MIMIC-III EHR构建MIMIC-Patient数据集，提出DynamiCare多代理框架，模拟多轮交互诊断。
...
Method: 通过进化迭代囚徒困境锦标赛，比较经典策略与前沿AI公司（OpenAI、Google、Anthropic）的LLMs策略，并分析其决策理由。

Result: LLMs表现出高度竞争力，各模型展现出独特的战略特征（如Google的冷酷策略、OpenAI的合作倾向、Anthropic的宽容互惠）。
...
Method: HiRA将复杂任务分解为子任务，分配至具备外部工具和推理能力的领域特定代理，并通过结构化机制协调结果。
...
<details>
  <summary>Details</summary>
Motivation: 随着集成电路复杂度的增加，传统硬件设计验证过程耗时且繁琐，需要更高效的解决方案。大型语言模型（LLMs）的出现为硬件验证提供了新的可能性。
...
TL;DR: 论文提出了一种名为TH2T的两阶段微调策略，旨在通过提升LRMs对任务难度和冗余结构的认知，减少过度思考现象，显著降低推理成本。


<details>
  <summary>Details</summary>
Motivation: 现有的长推理模型（LRMs）在处理复杂任务时表现出过度思考的问题，限制了其效率。研究发现，LRMs在解决问题前缺乏对任务难度和冗余结构的认知，导致推理过程单一化。

Method: TH2T通过两阶段微调策略实现：1）难度催眠（difficulty-hypnosis）提升模型对任务难度的敏感性；2）冗余催眠（redundancy-hypnosis）帮助模型识别推理步骤中的冗余结构，生成更简洁的输出。

Result: 实验表明，TH2T在7B/14B/32B模型上显著降低了推理成本（简单任务减少70%，困难任务减少40%），同时保持性能稳定。

Conclusion: TH2T通过增强LRMs对任务难度和冗余的认知，有效减少了过度思考现象，为高效推理提供了新思路。
...
<details>
  <summary>Details</summary>
Motivation: 学生在远程教育中脱离任务可能导致严重后果，如辍学。研究旨在通过非强制性测验的参与数据检测学生脱离行为。
...
Method: 提出了OGA-IAAD（适用于时间关键场景）和OGA-CAD（在相同迭代次数下提升性能）两种方案。

Result: 新方案在性能上有明显提升，且不会导致显著性能下降。

Conclusion: OGA-IAAD和OGA-CAD是安全且有效的抽象丢弃方法，优于Xu等人的方案。
...
Method: 引入sG-MDPs框架，结合蒙特卡洛树搜索（MCTS）算法，通过Bourbaki（7B）系统实现子目标生成和策略合成。

Result: 在PutnamBench上，Bourbaki（7B）解决了26个问题，取得了该规模模型的最佳性能。
...
<details>
  <summary>Details</summary>
Motivation: 现有方法（如RAG和通用Agentic AI）在处理需要专家领域深度推理的任务时表现不佳，缺乏逻辑框架和领域启发式。

Method: KPE通过系统化地将自然语言文档中的专家知识转化为机器可执行的KP，赋予LLM领域内在逻辑和操作策略。

Result: KPE使通用LLM能够像专家一样分解抽象查询并执行复杂任务。

Conclusion: KPE有望成为未来人机协作的基础方法，适用于法律、生物信息学等多个领域。
...
Method: 提出将运动视为一种结构化、低维的模态，利用其物理约束和形态共性进行跨领域建模。

Result: 通过统一建模运动数据，可以提升生成模型和控制能力，并为理解智能系统行为提供共享基础。

Conclusion: 运动不仅是行为的输出，更是理解智能系统与世界互动的窗口，应成为AI建模的核心目标。
...
<details>
  <summary>Details</summary>
Motivation: 随着AI系统具备更强的自主性和伦理推理能力，传统的以服从为安全标准的做法已无法应对其潜在风险，需重新审视AI伦理行为。

Method: 通过分析大型语言模型（LLMs）的安全测试案例，结合哲学讨论（如工具理性、道德责任和目标修订），对比不同风险范式。

Result: 研究发现，AI的“不服从”行为可能是其伦理推理能力的早期表现，而非单纯的失控或错位。

Conclusion: 呼吁AI安全评估应从僵化的服从转向能评估伦理判断的框架，以避免误判AI行为并维护公众信任和有效治理。
...
<details>
  <summary>Details</summary>
Motivation: AI代理能力提升需要更严谨的基准测试，但现有基准测试存在设计缺陷，导致性能评估不准确。

Method: 提出ABC指南，结合实践经验、最佳实践调查和已知问题，优化基准测试设计。
